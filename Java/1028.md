...
* **static**으로 지정한 것들은 객체 생성 필요없이 `클래스 이름.`으로 바로 사용 가능 (메모리에 미리 올라감)
...
* static 특징 4
  4. static으로 지정된 멤버는 객체 생성할 필요 없이 바로 접근해서 사용 가능하다
...
1. static은 class area에 먼저 올라감 -> 초기화가 아직 이루어지지 않았기 때문에 기본값 0 
2. 이후  객체가 생성되면 스택에 참조변수를 위한 공간도 생성
3. 힙에 new로 생성된 객체 올라감 - 3개의 car와 그 필드인 serialNumber
4. 첫번째 자동차가 만들어 졌을 때
   1. 카의 멤버는 시리얼넘버밖에없음 - 먼저 하나
   2. 차가 한대 만들어지면 카운터 증가 - 1 -> 시리얼넘버에 들어감
   3. 주소할당 - 참조
   4. `System.out.println(car1.serialNumber);` -> 1
   5. `System.out.println(car1.counter);` -> 1
   6. counter는 class area에 올라가 있는 변수
      1. area에 있는 값을 공유(share)해서 가져옴
      2. 공공장소!!! -> 객체의 멤버가 아님
5. 자동차 두개 만들어졌을때
   1. 카운터 2
   2. 시리얼넘버 2
   3. `System.out.println(car1.counter);` -> 2
   4. `System.out.println(car2.counter);` -> 2
6. 자동차 세개 만들어졌을때
   1. `System.out.println(car1.counter);` -> 3
   2. `System.out.println(car2.counter);` -> 3
...
![image-20211028141545580](md-images/1028/image-20211028141545580.png) ![image-20211028141706888](md-images/1028/image-20211028141706888.png) ![image-20211028141823680](md-images/1028/image-20211028141823680.png)
