...
public class FrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(command.equals("logout")) { //로그아웃 요청..결과 페이지 연결
			path=logout(request,response);
		}else if(command.equals("update")) { //로그아웃 요청..결과 페이지 연결
	...
	protected String update(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//login, 회원정보수정은 반드시....Session에 값을 바인딩....
	...
	protected String logout(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			HttpSession session=request.getSession();
			if(session.getAttribute("vo")!=null) { //로그인된 상태라면...그때 로그아웃 진행
				session.invalidate();
